                ...
                <div class="col-xs-12">
                    <div class="box">
                        <div class="box-header with-border">
                            <h3 class="box-title">Estados @if($miembro) - {{ $miembro->apellido }} {{ $miembro->nombre }} en la Unidad {{ $miembro->unidad->denominacion }} - {{ $miembro->unidad->sigla }}@endif</h3>
                            @if($miembro)<a class='pull-right btn btn-success' href="{{ route('miembro_estados.create', ['miembro_id' => $miembro->id]) }}">Cambiar</a>@endif
                        </div>
    ...
    <script>
        $(document).ready(function() {
            $('#example1').DataTable({
                ...
                scrollX: true,
                paging : true,
                "order": [[ 17, "desc" ]], // Ordenar por el tercer campo (apellido) de forma ascendente
                "ajax": {
                    "url": "{{ route('miembro_estados.dataTable') }}",
                    "type": "POST",
                    "data": function (d) {
                        d._token = '{{ csrf_token() }}'; // Agrega el token CSRF si estás usando Laravel
                        d.miembro_id = '{{ $miembro ? $miembro->id : '' }}'; // Enviar el ID del miembro como filtro
                ...
                columns: [
                    ...
                    {data: 'estudiante', name: 'estudiante'},
