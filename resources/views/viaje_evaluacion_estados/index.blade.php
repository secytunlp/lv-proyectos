                ...
                <div class="col-xs-12">
                    <div class="box">
                        <div class="box-header with-border">
                            <h3 class="box-title">Estados @if($viaje_evaluacion) - {{ $viaje_evaluacion->viaje->investigador->persona->apellido }} {{ $viaje_evaluacion->viaje->investigador->persona->nombre }} en la evaluación {{ $viaje_evaluacion->viaje->periodo->nombre }}@endif</h3>
                            @if($viaje_evaluacion)<a class='pull-right btn btn-success' href="{{ route('viaje_evaluacion_estados.create', ['viaje_evaluacion_id' => $viaje_evaluacion->id]) }}">Cambiar</a>@endif
                        </div>
    ...
    <script>
        $(document).ready(function() {
            $('#example1').DataTable({
                ...
                "ajax": {
                    "url": "{{ route('viaje_evaluacion_estados.dataTable') }}",
                    "type": "POST",
                    "data": function (d) {
                        d._token = '{{ csrf_token() }}'; // Agrega el token CSRF si estás usando Laravel
                        d.viaje_evaluacion_id = '{{ $viaje_evaluacion ? $viaje_evaluacion->id : '' }}'; // Enviar el ID del viaje_evaluacion como filtro
