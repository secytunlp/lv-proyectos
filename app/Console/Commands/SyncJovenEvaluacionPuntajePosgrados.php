...
class SyncJovenEvaluacionPuntajePosgrados extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'sync:jovenevaluacionpuntajeposgrados';
    ...
    public function handle()
    {
        ...
        DB::connection('mysql_origen')
            ->table('puntajeposgrado')



            ->selectRaw("`cd_puntajeposgrado` as id,`cd_evaluacion` as joven_evaluacion_id,`cd_modeloplanilla` as joven_evaluacion_planilla_id,`cd_posgradomaximo` as joven_evaluacion_planilla_posgrado_max_id
")
            ->orderBy('puntajeposgrado.cd_puntajeposgrado')
            ->chunk(1000, function ($rows) use (&$totalFilas, &$totalInsertadas, &$totalOmitidas, &$skippedRows){
                ...
                $data = collect($rows)->map(function ($row) use (&$skippedRows, &$totalOmitidas) {
                    ...
                    return [
                        ...
                        'joven_evaluacion_planilla_posgrado_max_id' => $row->joven_evaluacion_planilla_posgrado_max_id  ?: null,
                ...
                try {
                    ...
                    DB::connection('mysql')
                        ->table('joven_evaluacion_puntaje_posgrados')
                        ->upsert(
                            $data,
                            ['id'],
                            [
                                'joven_evaluacion_id','joven_evaluacion_planilla_id','joven_evaluacion_planilla_posgrado_max_id',
                                'updated_at'
                            ]
                        );
