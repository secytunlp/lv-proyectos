...
class SyncUnidadEstados extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'sync:unidadestados';
     ...
     * @var string
     */
    protected $description = 'Sincroniza cyt_unidad_tipo_estado desde DB origen a unidad_investigacion_estados';
    ...
    public function handle()
    {
       ...
       cyt_unidad_tipo_estado.fechaDesde as desde, cyt_unidad_tipo_estado.fechaHasta as hasta, cyt_unidad_tipo_estado.motivo as comentarios")
            ->orderBy('cyt_unidad_tipo_estado.oid')
            ->chunk(1000, function ($rows) use (&$totalFilas, &$totalInsertadas, &$totalOmitidas, &$skippedRows){
                ...
                $data = collect($rows)->map(function ($row) use (&$skippedRows, &$totalOmitidas) {
                    ...
                    $estadosValidos = [
                        'Creada','Recibida','Admitida','Aprobada'
                    ];
                ...
                try {
                    ...
                    DB::connection('mysql')
                        ->table('unidad_investigacion_estados')
                        ->upsert(
                            $data,
                            ['id'],
                            [
                                'unidad_id','user_id','user_name','estado',
