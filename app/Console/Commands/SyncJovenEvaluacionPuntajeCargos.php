...
class SyncJovenEvaluacionPuntajeCargos extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'sync:jovenevaluacionpuntajecargos';
            ...
            ->chunk(1000, function ($rows) use (&$totalFilas, &$totalInsertadas, &$totalOmitidas, &$skippedRows){
                ...
                $data = collect($rows)->map(function ($row) use (&$skippedRows, &$totalOmitidas) {
                    ...
                    return [
                        ...
                        'joven_evaluacion_planilla_cargo_max_id' => $row->joven_evaluacion_planilla_cargo_max_id  ?: null,
                ...
                try {
                    ...
                    DB::connection('mysql')
                        ->table('joven_evaluacion_puntaje_cargos')
                        ->upsert(
                            $data,
                            ['id'],
                            [
                                'joven_evaluacion_id','joven_evaluacion_planilla_id','joven_evaluacion_planilla_cargo_max_id',
                                'updated_at'
                            ]
                        );
