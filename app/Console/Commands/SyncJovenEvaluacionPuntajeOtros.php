...
class SyncJovenEvaluacionPuntajeOtros extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'sync:jovenevaluacionpuntajeotros';
    ...
    public function handle()
    {
        ...
        DB::connection('mysql_origen')
            ->table('puntajeantotros')



            ->selectRaw("`cd_puntajeantotros` as id,`cd_evaluacion` as joven_evaluacion_id,`cd_modeloplanilla` as joven_evaluacion_planilla_id,`cd_antotrosmaximo` as joven_evaluacion_planilla_otro_max_id,`nu_puntaje` as puntaje
")
            ->orderBy('puntajeantotros.cd_puntajeantotros')
            ->chunk(1000, function ($rows) use (&$totalFilas, &$totalInsertadas, &$totalOmitidas, &$skippedRows){
                ...
                $data = collect($rows)->map(function ($row) use (&$skippedRows, &$totalOmitidas) {
                    ...
                    return [
                        ...
                        'joven_evaluacion_planilla_otro_max_id' => $row->joven_evaluacion_planilla_otro_max_id  ?: null,
                ...
                try {
                    ...
                    DB::connection('mysql')
                        ->table('joven_evaluacion_puntaje_otros')
                        ->upsert(
                            $data,
                            ['id'],
                            [
                                'joven_evaluacion_id','joven_evaluacion_planilla_id','joven_evaluacion_planilla_otro_max_id','puntaje',
                                'updated_at'
                            ]
                        );
