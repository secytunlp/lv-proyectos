...
class SyncViajeEvaluacionPuntajeItems extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'sync:viajeevaluacionpuntajeitems';
            ...
            ->chunk(1000, function ($rows) use (&$totalFilas, &$totalInsertadas, &$totalOmitidas, &$skippedRows){
                ...
                $data = collect($rows)->map(function ($row) use (&$skippedRows, &$totalOmitidas) {
                    ...
                    return [
                        ...
                        'viaje_evaluacion_planilla_item_max_id' => $row->viaje_evaluacion_planilla_item_max_id  ?: null,
                ...
                try {
                    ...
                    DB::connection('mysql')
                        ->table('viaje_evaluacion_puntaje_items')
                        ->upsert(
                            $data,
                            ['id'],
                            [
                                'viaje_evaluacion_id','viaje_evaluacion_planilla_id','viaje_evaluacion_planilla_item_max_id','cantidad','puntaje',
                                'updated_at'
                            ]
                        );
