...
class SyncJovenEvaluacionPuntajeJustificacions extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'sync:jovenevaluacionpuntajejustificacions';
    ...
    public function handle()
    {
        ...
        DB::connection('mysql_origen')
            ->table('puntajeantjustificacion')



            ->selectRaw("`cd_puntajeantjustificacion` as id,`cd_evaluacion` as joven_evaluacion_id,`cd_modeloplanilla` as joven_evaluacion_planilla_id,`cd_antjustificacionmaximo` as joven_evaluacion_planilla_justificacion_max_id,`nu_puntaje` as puntaje
")
            ->orderBy('puntajeantjustificacion.cd_puntajeantjustificacion')
            ->chunk(1000, function ($rows) use (&$totalFilas, &$totalInsertadas, &$totalOmitidas, &$skippedRows){
                ...
                $data = collect($rows)->map(function ($row) use (&$skippedRows, &$totalOmitidas) {
                    ...
                    return [
                        ...
                        'joven_evaluacion_planilla_justificacion_max_id' => $row->joven_evaluacion_planilla_justificacion_max_id  ?: null,
                ...
                try {
                    ...
                    DB::connection('mysql')
                        ...
                        ->upsert(
                            ...
                            ['id'],
                            [
                                'joven_evaluacion_id','joven_evaluacion_planilla_id','joven_evaluacion_planilla_justificacion_max_id','puntaje',
                                'updated_at'
                            ]
                        );
