...
class SyncViajeEvaluacionPuntajeCargos extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'sync:viajeevaluacionpuntajecargos';
    ...
    public function handle()
    {
        ...
        DB::connection('mysql_origen')
            ...
            ->selectRaw("`cd_puntajecargo` as id,`cd_evaluacion` as viaje_evaluacion_id,`cd_modeloplanilla` as viaje_evaluacion_planilla_id,`cd_cargomaximo` as viaje_evaluacion_planilla_cargo_max_id
")
            ->orderBy('puntajecargo.cd_puntajecargo')
            ->chunk(1000, function ($rows) use (&$totalFilas, &$totalInsertadas, &$totalOmitidas, &$skippedRows){
                ...
                $data = collect($rows)->map(function ($row) use (&$skippedRows, &$totalOmitidas) {
                    ...
                    return [
                        ...
                        'viaje_evaluacion_planilla_cargo_max_id' => $row->viaje_evaluacion_planilla_cargo_max_id  ?: null,
                ...
                try {
                    ...
                    DB::connection('mysql')
                        ->table('viaje_evaluacion_puntaje_cargos')
                        ->upsert(
                            $data,
                            ['id'],
                            [
                                'viaje_evaluacion_id','viaje_evaluacion_planilla_id','viaje_evaluacion_planilla_cargo_max_id',
                                'updated_at'
                            ]
                        );
