                ...
                <div class="col-xs-12">
                    <div class="box">
                        <div class="box-header with-border">
                            ...
                            <?php endif; ?>
                            <a class='pull-right btn btn-success' href="<?php echo e(route('jovens.create')); ?>">Nueva</a>
    ...
    <script>
        $(document).ready(function() {
            ...
            let actionsColumn = {
                    ...
                    "render": function (data, type, row) {
                        ...
                        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('joven_estado-listar')): ?>
                            actionsHtml += '<a href="<?php echo e(route("joven_estados.index")); ?>?joven_id=' + row.id + '" alt="Estados" title="Estados" style="margin-right: 5px;"><i class="fa fa-tasks"></i></a>';
                        <?php endif; ?>
                        if (row.estado == 'Creada') {
                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('solicitud-editar')): ?>
                                actionsHtml += '<a href="<?php echo e(route("jovens.edit", ":id")); ?>" alt="Modificar solicitud" title="Modificar solicitud" style="margin-right: 5px;"><span class="glyphicon glyphicon-edit"></span></a>'.replace(':id', row.id);
                            <?php endif; ?>
                            ...
                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('solicitud-eliminar')): ?>
                                actionsHtml += '<form id="delete-form-' + row.id + '" method="post" action="<?php echo e(route('jovens.destroy', '')); ?>/' + row.id + '" style="display: none">';
                        ...
                        actionsHtml += '<a href="<?php echo e(route("jovens.solicitud-pdf")); ?>?joven_id=' + row.id + '" alt="Descargar PDF" title="Descargar PDF" target="_blank" style="margin-right: 5px;"><i class="fa fa-file-pdf"></i></a>';

                        actionsHtml += '<a href="<?php echo e(route("jovens.archivos")); ?>?joven_id=' + row.id + '" alt="Descargar archivos" title="Descargar archivos" target="_blank" style="margin-right: 5px;"><i class="fa fa-download"></i></a>';


                        if (row.estado == 'Creada') {
                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('solicitud-editar')): ?>

                                actionsHtml += '<form id="send-form-' + row.id + '" method="post" action="<?php echo e(route('jovens.enviar', '')); ?>/' + row.id + '" style="display: none">';
                            actionsHtml += '<?php echo e(csrf_field()); ?>';
                        ...
                        if (row.estado == 'Recibida') {
                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('solicitud-admitir')): ?>
                                actionsHtml += '<form id="admit-form-' + row.id + '" method="post" action="<?php echo e(route('jovens.admitir', '')); ?>/' + row.id + '" style="display: none">';
                            actionsHtml += '<?php echo e(csrf_field()); ?>';
                            ...
                            <?php endif; ?>
                                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('solicitud-rechazar')): ?>
                                actionsHtml += '<a href="<?php echo e(route("jovens.rechazar", ":id")); ?>" alt="Rechazar" title="Rechazar" style="margin-right: 5px;"><i class="fa fa-times-circle"></i></a>'.replace(':id', row.id);
                            <?php endif; ?>
                        }
                        ...
                        if ((row.estado == 'Admitida') || (row.estado == 'Otorgada-No-Rendida') || (row.estado == 'En evaluación') || (row.estado == 'No otorgada') || (row.estado == 'Evaluada') || (row.estado == 'Otorgada-Rendida') || (row.estado == 'Otorgada-Renunciada') || (row.estado == 'Otorgada-Devuelta')) {
                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('evaluacion-listar')): ?>
                                actionsHtml += '<a href="<?php echo e(route("joven_evaluacions.index")); ?>?joven_id=' + row.id + '" alt="Evaluaciones" title="Evaluaciones" style="margin-right: 5px;"><i class="fa fa-th-list"></i></a>';
                            <?php endif; ?>
                        }
                        if (row.estado == 'En evaluación') {
                            if (row.evaluacion_estado == 'Recibida') {
                                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('evaluacion-evaluar')): ?>
                                    actionsHtml += '<form id="accept-form-' + row.id + '" method="post" action="<?php echo e(route('joven_evaluacions.aceptar', '')); ?>/' + row.id + '" style="display: none">';
                                actionsHtml += '<?php echo e(csrf_field()); ?>';
                                ...
                                actionsHtml += '<a href="<?php echo e(route("joven_evaluacions.rechazar", ":id")); ?>" alt="Rechazar" title="Rechazar" style="margin-right: 5px;"><i class="fa fa-times-circle"></i></a>'.replace(':id', row.id);
                                <?php endif; ?>
                            }
                            if (row.evaluacion_estado == 'En evaluación') {
                                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('evaluacion-evaluar')): ?>

                                    actionsHtml += '<a href="<?php echo e(route("joven_evaluacions.evaluar", ":id")); ?>" alt="Evaluar" title="Evaluar" style="margin-right: 5px;"><i class="fa fa-clipboard-check"></i></a>'.replace(':id', row.id);

                                    actionsHtml += '<form id="sendEval-form-' + row.id + '" method="post" action="<?php echo e(route('joven_evaluacions.send', '')); ?>/' + row.id + '" style="display: none">';
                                    actionsHtml += '<?php echo e(csrf_field()); ?>';
                                    actionsHtml += '</form>';
                        ...
                        if (row.evaluacion_estado == 'En evaluación' || row.evaluacion_estado == 'Evaluada') {
                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('evaluacion-evaluar')): ?>
                                ...
                                actionsHtml += '<a href="<?php echo e(route("joven_evaluacions.evaluacion-pdf")); ?>?joven_id=' + row.id + '" alt="Evaluación" title="Evaluación" target="_blank" style="margin-right: 5px;"><i class="fa fa-file-pdf"></i></a>';
                            <?php endif; ?>
                        }
                        return actionsHtml;
            ...
            var table = $('#example1').DataTable({
                ...
                "ajax": {
                    "url": "<?php echo e(route('jovens.dataTable')); ?>",
                    "type": "POST",
                    "data": function (d) {
            ...
            $('#exportar-datos').click(function () {
                ...
                window.location.href = '<?php echo e(route('jovens.exportar')); ?>?' + $.param(filtros);
            });
        });
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/sicadi/resources/views/jovens/index.blade.php ENDPATH**/ ?>